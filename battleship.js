...
const Battleship = () => {
  ...
  useEffect(() => {
    ...
    if (ai2x1_2 === 12) {
      console.log("ai2x1_2 reached 12");
      setShip5Sunk(true);
    }
    if (ai2x1_3 === 12) {
      console.log("ai2x1_3 reached 12");
      setShip6Sunk(true);
    }
    if (ai1x1_1 === 12) {
      console.log("ai1x1_1 reached 12");
      setShip7Sunk(true);
    }
    if (ai1x1_2 === 12) {
      console.log("ai1x1_2 reached 12");
      setShip8Sunk(true);
    }
    if (ai1x1_3 === 12) {
      console.log("ai1x1_3 reached 12");
      setShip9Sunk(true);
    }
    if (ai1x1_4 === 12) {
      console.log("ai1x1_4 reached 12");
      setShip10Sunk(true);
    }
  }, [
    ai4x1_1,
      ...
      if (turn === "player") {
        ...
        if (!aiShots.includes(`${row}-${col}`)) {
          ...
          if (shotResult === "hit") {
            ...
            console.log(
              `ship${shipSize.id}Sunk:`,
              eval(`ship${shipSize.id}Sunk`)
            );
            if (
              shipSize.size === 1 ||
  ...
  const renderShipStatus = (shipSize, count, sunk) => {
    const shipCells = Array.from({ length: shipSize }, (_, index) => (
      <span
        key={index}
        style={{ color: sunk ? "grey" : count > 0 ? "black" : "white" }}
      >
        {count > 0 ? "⬛" : "⬜"}
      </span>
    ));
  ...
  const renderShipsStatus = () => {
    return Object.entries(shipsAvailable).map(([shipSize, count]) => {
      ...
      return (
        <div key={shipSize}>{renderShipStatus(shipNumber, count, sunk)}</div>
      );
    });
  };
                ...
                <tr key={rowIndex}>
                  {row.map((cell, colIndex) => (
                    <td
                      ...
                      onMouseEnter={() =>
                        handlePregameCellHover(rowIndex, colIndex)
                      }
                  ...
                  <tr key={rowIndex}>
                    {row.map((cell, colIndex) => (
                      <td
                        ...
                        onClick={() =>
                          !pregame && handleShoot(rowIndex, colIndex, "player")
                        }
                      ></td>
                    ))}
                  </tr>
