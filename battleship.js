...
const Battleship = () => {
    ...
    const [selectedShipIsHorizontal, setSelectedShipIsHorizontal] =
    useState(true);
    ...
    const [aiBoardHits, setAiBoardHits] = useState([]);
    ...
    const [aiShipCoordinates, setAIShipCoordinates] = useState([]);
    ...
    const handlePregameCellClick = (row, col) => {
        if (pregame && currentTurn === "player" && selectedShipSize) {
            ...
            if (canPlaceShip(row, col, shipSize, isHorizontal)) {
                ...
                alert("you can't place ships there!");
            }
        }
    };
    const closePopup = () => {
    ...
    const canPlaceShip = (row, col, size, isHorizontal) => {
        ...
        for (let i = 0; i < size; i++) {
            if (isHorizontal) {
                if (
                    playerBoard[row] &&
                    (playerBoard[row][col + i] !== null ||
                        hasNeighboringShip(row, col + i))
                ) {
                    return false;
                }
            } else {
                if (
                    playerBoard[row + i] &&
                    (playerBoard[row + i][col] !== null ||
                        hasNeighboringShip(row + i, col))
                ) {
                    return false;
                }
            }
    ...
    const generateRandomShips = () => {
        ...
        const newAIShipCoordinates = [];
        placeRandomShip(newAiBoard, "4x1", 1, newAIShipCoordinates);
        placeRandomShip(newAiBoard, "3x1", 2, newAIShipCoordinates);
        placeRandomShip(newAiBoard, "3x1", 3, newAIShipCoordinates);
        placeRandomShip(newAiBoard, "2x1", 4, newAIShipCoordinates);
        placeRandomShip(newAiBoard, "2x1", 5, newAIShipCoordinates);
        placeRandomShip(newAiBoard, "2x1", 6, newAIShipCoordinates);
        placeRandomShip(newAiBoard, "1x1", 7, newAIShipCoordinates);
        placeRandomShip(newAiBoard, "1x1", 8, newAIShipCoordinates);
        placeRandomShip(newAiBoard, "1x1", 9, newAIShipCoordinates);
        placeRandomShip(newAiBoard, "1x1", 10, newAIShipCoordinates);
        setAIShipCoordinates(newAIShipCoordinates);
        return newAiBoard;
    };

    const placeRandomShip = (board, shipSize, id, aiShipCoordinates) => {
        ...
        do {
            ...
            !canPlaceShipAIAvoidOverlap(
                row,
                col,
                parseInt(shipSize.split("x")[0]),
                isHorizontal,
                board
            )
        );

        const shipCoordinatesForThisShip = [];

        for (let i = 0; i < parseInt(shipSize.split("x")[0]); i++) {
            let currentRow = isHorizontal ? row : row + i;
            let currentCol = isHorizontal ? col + i : col;

            shipCoordinatesForThisShip.push(`${currentRow}-${currentCol}`);

            aiShipCoordinates.push({
                shipId: id,
                coordinate: `${currentRow}-${currentCol}`,
            });
    ...
    const handleAImove = () => {
        ...
        do {
            ...
            aiShotHistory.some((coord) => coord.row === aiRow && coord.col === aiCol)
        );
            ...
            if (turn === "player") {
                ...
                if (!aiShots.includes(`${row}-${col}`)) {
                    ...
                    if (shotResult === "hit") {
                        ...
                        const shipId = getAIShipId(coordinate);
                        const isShipSunk = shipId ? eval(`ship${shipId}Sunk`) : false;

                        if (shipSize.id === 1) {
                            setAI4x1_1((prevValue) => prevValue + 3);
                            if (ai4x1_1 === 9) {
                                setShip1Sunk(true);
                                const firstCoordinates = aiShipCoordinates.slice(0, 4);
                                firstCoordinates.forEach(({
                                    coordinate
                                }) => {
                        ...
                        if (shipSize.id === 2) {
                            ...
                            if (ai3x1_1 == 8) {
                                setShip2Sunk(true);
                                const firstCoordinates = aiShipCoordinates.slice(4, 7);
                                firstCoordinates.forEach(({
                                    coordinate
                                }) => {
                                    const [row, col] = coordinate.split('-').map(Number);
                        ...
                        if (shipSize.id === 3) {
                            ...
                            if (ai3x1_2 == 8) {
                                setShip3Sunk(true);
                                const firstCoordinates = aiShipCoordinates.slice(7, 10);
                                firstCoordinates.forEach(({
                                    coordinate
                                }) => {
                                    const [row, col] = coordinate.split('-').map(Number);
                        ...
                        if (shipSize.id === 4) {
                            ...
                            if (ai2x1_1 == 6) {
                                setShip4Sunk(true);
                                const firstCoordinates = aiShipCoordinates.slice(10, 12);
                                firstCoordinates.forEach(({
                                    coordinate
                                }) => {
                                    const [row, col] = coordinate.split('-').map(Number);
                        ...
                        if (shipSize.id === 5) {
                            ...
                            if (ai2x1_2 == 6) {
                                setShip5Sunk(true);
                                const firstCoordinates = aiShipCoordinates.slice(12, 14);
                                firstCoordinates.forEach(({
                                    coordinate
                                }) => {
                                    const [row, col] = coordinate.split('-').map(Number);
                        ...
                        if (shipSize.id === 6) {
                            ...
                            if (ai2x1_3 == 6) {
                                setShip6Sunk(true);
                                const firstCoordinates = aiShipCoordinates.slice(14, 16);
                                firstCoordinates.forEach(({
                                    coordinate
                                }) => {
                                    const [row, col] = coordinate.split('-').map(Number);
                        ...
                        if (
                            shipSize.size === 1 ||
                            (shipSize.size === 2 && shipSize.id == 4 && ai2x1_1 == 6) ||
                            (shipSize.size === 2 && shipSize.id == 5 && ai2x1_2 == 6) ||
                            (shipSize.size === 2 && shipSize.id == 6 && ai2x1_1 == 6) ||
                            (shipSize.size === 3 && shipSize.id == 2 && ai3x1_1 == 8) ||
                            (shipSize.size === 3 && shipSize.id == 3 && ai3x1_2 == 8) ||
                            (shipSize.size === 4 && shipSize.id == 1 && ai4x1_1 == 9)
                        ) {
                ...
                if (!playerShots.includes(`${playerRow}-${playerCol}`)) {
                    ...
                    if (shotResult === "hit") {
                        ...
                        setAIhits(AIHits + 1);
                        setAiBoardHits((prevHits) => [...prevHits, `${row}-${col}`]);
                        if (
                            shipSize.size === 1 ||
                            (shipSize.size === 2 && shipSize.id == 4 && ai2x1_1 == 6) ||
                            (shipSize.size === 2 && shipSize.id == 5 && ai2x1_2 == 6) ||
    ...
    const renderShipStatus = (shipSize, count, sunk) => {
        const shipCells = Array.from({
            ...
            style = {
                {
                    color: sunk ? "grey" : count > 0 ? "black" : "white"
                }
            } >
            {
                count > 0 ? "⬛" : "⬜"
    ...
    const getAIShipId = (coordinate) => {
        const matchingShip = aiShipCoordinates.find((ship) => ship.coordinate === coordinate);
        return matchingShip ? matchingShip.shipId : null;
    };
        ...
        {row.map((cell, colIndex) => (
          <td
            ...
            style={{
              ...
              textAlign: "center",
              background:
  aiBoardHits.includes(`${rowIndex}-${colIndex}`)
    ? "#ff8080" 
    : cell === "1x1" ||
      cell === "2x1" ||
      cell === "3x1" ||
      ...
      : hoveredCell &&
        (hoveredCell.includes(`${rowIndex}-${colIndex}`) ||
          hoveredCell.includes(`${row}-${colIndex}`))
      ? "lightcyan"
      : cell === "hit"
      ? "linear-gradient(to top left, transparent calc(50% - 1px), red 50%, transparent calc(50% + 1px)), linear-gradient(to top right, transparent calc(50% - 1px), red 50%, transparent calc(50% + 1px))"
      : "transparent",
              ...
              <div>
                ...
                >
                  <img
                    ...
                    src={shuffle}
                    style={{ width: 23, height: 23, postion: "relative" }}
                  />
                  <div
                    style={{
                      marginLeft: "10px",
                ...
                >
                  <li onClick={() => handleShipSelection("4x1")}>
                    ⬛⬛⬛⬛: {shipsAvailable["4x1"]}
                  </li>
                  <li onClick={() => handleShipSelection("3x1")}>
                    ⬛⬛⬛: {shipsAvailable["3x1"]}
                  </li>
                  <li onClick={() => handleShipSelection("2x1")}>
                    ⬛⬛: {shipsAvailable["2x1"]}
                  </li>
                  <li onClick={() => handleShipSelection("1x1")}>
                    ⬛: {shipsAvailable["1x1"]}
                  </li>
                </ul>
              </div>
      ...
      return (
        <td
          ...
          style={{
            width: "30px",
            height: "30px",
            border: `1px solid ${isShipSunk ? "#ddd" : "#ddd"}`,
            textAlign: "center",
            background: aiShots.includes(coordinate)
              ? cell
                ? isShipSunk
                  ? "#FF8080" 
                  : "linear-gradient(to top left, transparent calc(50% - 1px), red 50%, transparent calc(50% + 1px)), linear-gradient(to top right, transparent calc(50% - 1px), red 50%, transparent calc(50% + 1px))"
      ...
      {gameOutcome && (
        ...
        >
          {gameOutcome && (
            ...
            >
              <div>{gameOutcome === "win" ? "You won!" : "You lost!"}</div>
              <div>
                <button
                  style={{
                    marginTop: "20px",
  ...
  >
    ...
    >
            <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
              <button onClick={() => changeLanguage('pl')} style={{ cursor: 'pointer', border: 0, backgroundColor: 'transparent'  }}>
	  <img src={pl} alt="Flaga Polski" style={{ width: '30px', height: '20px' }} />
              </button>
              <button onClick={() => changeLanguage('en')} style={{ cursor: 'pointer', border: 0, backgroundColor: 'transparent' }}>
	  <img src={us} alt="Amerykańska Flag" style={{ width: '30px', height: '20px' }} />
              </button>
            </div>
            <div>
              <h2>{language === 'en' ? 'How to Play' : 'Jak grać'}</h2>
              <p>
                <b>{language === 'en' ? '1.' : '1.'}</b> {language === 'en' ? 'To place ships on your board, click on the ship size underneath it, and then select where to place it.' : 'Aby umieścić statki na swojej planszy, kliknij na rozmiar statku poniżej planszy, a następnie wybierz miejsce, gdzie chcesz go umieścić.'}
                <br />
                {language === 'en' ? 'To rotate the ship, press "R". If you want to place ships randomly, click the shuffle icon under the board.' : 'Aby obrócić statek, naciśnij "R". Jeśli chcesz umieścić statki losowo kliknij w ikonkę przetasowania pod planszą'}

              </p>
              <p><b>{language === 'en' ? '2.' : '2.'}</b> {language === 'en' ? 'Click the "Start Game" button to begin the game.' : 'Kliknij przycisk "Start Game", aby rozpocząć grę.'}</p>
              <p>
                <b>{language === 'en' ? '3.' : '3.'}</b> {language === 'en' ? 'Click on the opponent\'s board to shoot; hits are marked with red ' : 'Kliknij na planszę przeciwnika, aby strzelać; trafienia są oznaczone czerwonym '}
                <span style={{ color: 'red' }}>X</span>
                {language === 'en' ? ', and misses with grey dots (•).' : ', a chybienia szarymi kropkami (•).'}
              </p>
			  <p>
                <b>{language === 'en' ? '4.' : '4.'}</b> 
				{language === 'en' ? 'When you sink your enemy\'s ship it will change it\'s color to light red' : 'Jeśli zatopisz statek przeciwnika, zmieni on kolor na jasno czerwony '}
              </p>
              <p><b>{language === 'en' ? '5.' : '5.'}</b> {language === 'en' ? 'When either you or your opponent sinks all of their enemy\'s ships, they win.' : 'Gdy zatopisz wszystkie statki przeciwnika zanim on zatopi twoje wygrywasz, w przecinym razie przegrywasz'}</p>
            </div>
            <button
              className="close-btn"
              style={{
